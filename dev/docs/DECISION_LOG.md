...
### Fase 2 — Correção de 3 Bugs de Sincronização + Testes de Regressão
F2 | FIX | Bug 1: `renderReadingMode` aplicava `.active` no token `startIdx` antes do `onboundary` — removida aplicação antecipada; primeiro highlight agora vem exclusivamente de `highlightToken` via `onboundary` | Realce prematuro coloria palavra antes de ser falada; agora sincronia perfeita com áudio | balabolka_web.html
F2 | FIX | Bug 2: `findStartToken` retornava índice de whitespace quando cursor em linha vazia — adicionada `skipWhitespaceTokens(tokens, startIdx)` que avança até `/\S/`; integrada em ambos branches de `playSpeech` | Cursor em `\n\n` ou espaços iniciava leitura/colorindo whitespace; agora pula para próxima palavra real | balabolka_web.html
F2 | FIX | Bug 3: `clearReadingView()` usava `state.texts[]` ou `editor.innerText` tokenizado (com spans), perdendo texto real e posicionando cursor no início — reescrita para extrair texto via `.textContent` dos spans antes de limpar, calcular `cursorCharOffset` via `lastReadTokenIdx`, e restaurar cursor via tree-walk | Edição no modo leitura parada resetava texto para vazio/posição 0; agora preserva texto e cursor na posição onde parou | balabolka_web.html
F2 | ADD | Módulo `skipWhitespaceTokens(tokens, startIdx)` — avança índice até primeiro token não-whitespace | Necessário para Bug 2; reutilizável em retomada e início normal | balabolka_web.html
F2 | ADD | Módulo `findStartToken(text, cursorOffset)` — mapeia offset de caractere para índice de token (extraction da lógica inline de `playSpeech`) | Separado para testabilidade e clareza; usado por `playSpeech` | balabolka_web.html
F2 | ADD | 3 testes automatizados de regressão: Bug 3 (edição não reseta), Bug 2 (linha vazia pula whitespace), Bug 1 (`.active` não antecipado) | Cobertura completa dos 3 bugs corrigidos; execução síncrona para Bug 1 evita race condition | tests/highlight.spec.js
F2 | MOD | Teste "Bug 1" reescrito para verificação síncrona via `window.__activeCountImmediatelyAfterPlay` | Teste original tinha race condition (5ms vs 10ms do mock); agora valida estado exato pós-`playSpeech` | tests/highlight.spec.js
F2 | TECH | Total de testes: 8/8 passando (5 Fase 1 + 3 Fase 2) | Suite completa valida todos bugs críticos + invariantes | tests/highlight.spec.js