...
> Última atualização: Fase 1 | 03/07/2026
...
|--------|---------|------------------|-------|
| `state` | `balabolka_web.html` (var) | Objeto global com tabs, texts, activeTab, rate, pitch, volume, selectedVoiceName, colors, font, zoom, buttonSize, isMd, voices, ttsSupported, isPlaying, isPaused, isDirty, isRecording, renaming, findText, replaceText, findCase, activeMenu, dragging, readingTokens, readingStartIdx, currentTokenIdx, originalMdTexts, isRenderedMd, isInReadingView, lastReadTokenIdx, keepAliveTimer | Fase 1 |
...
| `renderReadingMode(tokens, startIdx)` | `balabolka_web.html` (fn) | `(tokens: string[], startIdx: number): void` — substitui innerHTML por spans `.token` coloridos, preserva scrollTop e scrollHeight | Fase 1 |
| `clearReadingView()` | `balabolka_web.html` (fn) | `(): void` — remove spans, restaura innerText, reativa contentEditable, esconde hint | Fase 1 |
...
2. Clicar "Ler" → `playSpeech()` captura cursor (`getCursorOffset()`), tokeniza texto original com `\n`, renderiza spans coloridos (`renderReadingMode`), divide em chunks de ~3000 chars
3. `SpeechSynthesisUtterance.onboundary` → `highlightToken(idx)` usando `chunkOffset + e.charIndex` mapeado via `tokenMap`
4. `onend` → próximo chunk ou `restoreEditorMode()` (preserva spans se algo foi lido, reativa contentEditable)
5. Modo leitura parada (`isInReadingView === true`): clique em token atualiza `lastReadTokenIdx` sem limpar spans; duplo clique ou digitação (`input`) → `clearReadingView()`
...
|-------|---------|----------------------|---------|
| Highlight | `tests/highlight.spec.js` | 5 testes (Persistência, Scroll, Edição limpa realce, Precisão quebra de linha, Retomada por clique) | `npx playwright test tests/highlight.spec.js` |
